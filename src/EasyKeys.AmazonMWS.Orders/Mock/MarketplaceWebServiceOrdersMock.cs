...
namespace EasyKeys.AmazonMWS.Orders.Model.Mock
{
    /// <summary>
    ...
    public class MarketplaceWebServiceOrdersMock : IMarketplaceWebServiceOrders
    {
        ...
        private T NewResponse<T>() where T : IMWSResponse
        {
            ...
            try
            {
                xmlIn = Assembly.GetAssembly(GetType()).GetManifestResourceStream(typeof(T).FullName + ".xml");
                var xmlInReader = new StreamReader(xmlIn);
                var xmlStr = xmlInReader.ReadToEnd();

                var reader = new MwsXmlReader(xmlStr);
                var obj = (T)Activator.CreateInstance(typeof(T));
                obj.ReadFragmentFrom(reader);
                obj.ResponseHeaderMetadata = new ResponseHeaderMetadata("mockRequestId", "A,B,C", "mockTimestamp", 0d, 0d, default(DateTime));
                return obj;
            }
            catch (Exception e)
            {
