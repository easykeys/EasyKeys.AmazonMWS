...
namespace EasyKeys.AmazonMWS.Runtime
{
    public class MwsUtil
    {
        ...
        public static string ReplaceAll(string s, Regex p, string r)
        {
            ...
            var buf = new StringBuilder(n + 12);
            var k = 0;
            do
            {
        ...
        public static string Sign(string data, string key, string algorithm)
        {
            try
            {
                var alg = KeyedHashAlgorithm.Create(algorithm.ToUpper());
         ...
         */
        public static string SignParameters(
            Uri serviceUri,
        ...
        public static bool HasStandardPort(Uri uri)
        {
            ...
            var standardPort = schema.Equals("https") ? 443 : 80;
        ...
        private static string CalculateStringToSignV1(IDictionary<string, string> parameters)
        {
            ...
            var pairs = sorted.GetEnumerator();
            ...
            IDictionary<string, string> parameters)
        {
            ...
            data.Append("\n");
            var uri = serviceUri.LocalPath;
            data.Append(UrlEncode(uri, true));
            data.Append("\n");
            var pairs = parameters.GetEnumerator();
            var isFirst = true;
            while (pairs.MoveNext())
            {
                if (isFirst)
